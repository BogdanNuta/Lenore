        <div class="panel">
          <div class="carousel">
            <ul class="grid sidebar-grid" id="up-next-videos">
			<?php
			$tcnt = 0;
			$cnt = 0;
			$set_played = true;
            ...
            $commentCountUpNextJson = curl_exec($curl_handle);
            curl_close($curl_handle);
            
            $commentCountsUpNext = json_decode($commentCountUpNextJson, true);
			...
			foreach ($videos as $video) {
				...
				$li_class = "";
				if($cnt == 2 || $cnt == 5) $li_class = 'last-in-row';
				if($cnt == 3) $li_class = 'first-in-row';
								
				if($set_played) $li_class .= ' played';
			...
			<li><ul>
			<?php } ?>
			<li class="<?php echo $li_class; ?>" id="v-<?php echo $video->id; ?>">
				...
				<div class="fb_ltr" style="display:none;" id="comments-list-<?php echo $video->id; ?>">
					<?php 
    				    ...
    				    if(isset($commentCountsUpNext[$commentVideoUrl]))
                        {
                            if(isset($commentCountsUpNext[$commentVideoUrl]["comments"]))
                            {
                                $commentCount = $commentCountsUpNext[$commentVideoUrl]["comments"];
                            }
                        }
			...
			<?php if($cnt == 5 || sizeof($videos) == ($tcnt+2)) { ?>
			</ul></li>
			<?php } ?>
			<?php
				if($cnt == 5) {
            ...
            </ul> <!-- //grid sidebar-grid: up next -->

			<?php echo $scroll_links_upnext; ?>

          </div> <!-- //carousel: up next -->
        </div> <!-- //panel: up next -->
